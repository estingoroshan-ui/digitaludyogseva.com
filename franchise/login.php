<?php
$page_title = "Franchise Partner Login | Digital Udyog Seva";
require_once __DIR__ . '/../config/app.php';
require_once __DIR__ . '/../includes/auth.php';
require_once __DIR__ . '/../includes/csrf.php';

$error = '';
$email_val = $_POST['email'] ?? 'franchise@digitaludyogseva.com';
$password_val = $_POST['password'] ?? 'Franchise@123';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $res = login_user($email, $password, 'franchise');
        if ($res['status']) {
            header('Location: ' . BASE_URL . 'franchise/index.php');
            exit;
        } else {
            $error = $res['message'];
        }
...
<head>
    ...
    <style>
        :root {
            ...
            --accent-glow: radial-gradient(circle at 50% 0%, rgba(245, 158, 11, 0.25) 0%, transparent 70%);
        }
        body {
...
<body>
    ...
    <div class="login-card">
        ...
        <div class="text-center mb-4">
            <div class="brand-badge-franchise mb-3">
                <i class="bi bi-building-check"></i> DUS FRANCHISE NETWORK
            </div>
            <h2 class="font-heading fw-bold mb-1">Franchise Partner Portal</h2>
            <p class="text-secondary small mb-0">Manage clients, submit applications & track commission wallet</p>
        </div>
        ...
        <form action="" method="POST">
            ...
            <div class="mb-3">
                <label class="form-label small fw-bold text-light">Franchise Mobile or Email</label>
                <div class="input-group">
                    <input type="text" name="email" class="form-control form-control-custom" required value="<?php echo htmlspecialchars($email_val); ?>" placeholder="franchise@digitaludyogseva.com">
                </div>
            </div>
            ...
            <button type="submit" class="btn btn-login w-100 mb-3">
                Franchise Sign In <i class="bi bi-arrow-right-short fs-5 ms-1"></i>
            </button>
        </form>
